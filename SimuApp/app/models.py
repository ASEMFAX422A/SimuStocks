...
class DataEntry(db.EmbeddedDocument):
    # Embedded document for storing price and timestamp data
    price = db.FloatField()
    timestamp = db.DateTimeField(default=datetime.now)

class ObjectChangelog(db.EmbeddedDocument, SimuBaseModell):
    # Embedded document for storing object changelog data
...
class Stocks(db.DynamicDocument):
    # Collection for storing stock data
...
class AppRun(db.DynamicDocument):
    # Collection for storing application run data
    meta = {"collection": "app_run"}
    time = db.DateTimeField(default=datetime.now)
    version = db.StringField(required=True)
    version_id = db.StringField(required=True)
    mode = db.StringField(required=True)

class ApplicationLog(db.DynamicDocument):
    # Collection for storing application log data
    meta = {"collection": "application_logs"}
    time = db.DateTimeField(default=datetime.now)
    levelname = db.StringField(required=True)
...
class Prices(db.DynamicDocument):
    # Collection for storing price data
